...
const RichTextEditor = ({ blog }: { blog: IBlogPost }) => {
  ...
  useEffect(() => {
    ...
    if (quillRef.current) {
      ...
      setInitialImages(Array.from(quillRef.current.root.querySelectorAll('img')).map(img => img.src));
    }
  }, [blog]);
  useEffect(() => {
   if (images.length > 4) {
     alert('You can only upload 4 images per blog post');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (quillRef.current) {
        ...
        const currentImages = Array.from(quillRef.current.root.querySelectorAll('img')).map(img => img.src);
        const removedImages = initialImages.filter(img => !currentImages.includes(img));
        
        for (const imageUrl of removedImages) {
         const key = imageUrl.split('https://dnyvrvurgen90.cloudfront.net/')[1];
        ...
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_GATEWAY!}/update?id=${blog._id}`, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': process.env.NEXT_PUBLIC_API_KEY!,
            },
            body: JSON.stringify({ title,description, slug, thumbNail : thumbNail === '' ? currentImages[0] : thumbNail, content: updatedContent, author: { name: 'Hridaya', profileImageUrl: "https://thriveholistic.in/hridaya.png" } }),
          })
  ...
  return (
   <form onSubmit={handleSubmit} className='max-w-2xl max-h-fit relative w-full flex-shrink-0  border rounded-md border-neutral-800 '>
     <div className='border-b border-red-800'>
      ...
      <div ref={editorRef} className='max-h-[70vh] border overflow-y-auto' />
      <button type="submit"  className='border rounded-full px-8 py-2 bg-red-700 text-white font-medium'>Update Post</button>
    </form>
  );
};
